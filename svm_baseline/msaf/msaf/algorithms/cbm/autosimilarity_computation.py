...
def l2_normalize_barwise(an_array):
    ...
    with warnings.catch_warnings():
        ...
        an_array_T = np.where(
            np.isnan(an_array_T), 1e-10, an_array_T
...
def get_autocorrelation_autosimilarity(an_array, normalize=True):
    ...
    if type(an_array) is list:
        ...
        this_array = an_array
    this_array = this_array - this_array.mean(
        axis=0
    )  # centering, i.e. subtracting the average value row-wise
    if normalize:
        this_array = l2_normalize_barwise(this_array)
    return this_array @ this_array.T
...
def get_rbf_autosimilarity(an_array, gamma=None, normalize=True):
    ...
    if gamma == None:
        gamma = get_gamma_std(
            this_array, scaling_factor=1, no_diag=True, normalize=normalize
        )
    if normalize:
        this_array = l2_normalize_barwise(this_array)
